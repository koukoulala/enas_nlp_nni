...
def DEFINE_list(name, default_value, doc_string):
  tf.app.flags.DEFINE_list(name, default_value, doc_string)
  global user_flags
  user_flags.append(name)
...
def get_train_ops(
    ...
    if lr_decay_scheme == "cosine":
        ...
        curr_epoch = train_step * lr_decay_epoch_multiplier // num_train_batches
    ...
    elif lr_decay_scheme == "exponential":
    ...
    elif lr_decay_scheme == "noam":
        learning_rate = (lr_model_d ** -0.5) * tf.minimum(
            tf.to_float(train_step) * (tf.to_float(lr_warmup_steps) ** -1.5),
            tf.to_float(train_step) ** -0.5)
        learning_rate = learning_rate * lr_init
    elif lr_decay_scheme == "auto":
        learning_rate = tf.placeholder(tf.float32, shape=(), name="learning_rate")
    else:
        raise ValueError(
            "Unknown learning rate decay scheme {}".format(lr_decay_scheme))

    if lr_decay_scheme != "noam" and lr_warmup_val is not None:
    ...
    if optim_algo == "momentum":
        ...
        opt = tf.train.AdamOptimizer(learning_rate, beta1=0.9, beta2=0.999, epsilon=1e-3,
                                     use_locking=True)
    elif optim_algo == "rmsprop":
        opt = tf.train.RMSPropOptimizer(
            learning_rate, decay=0.5, momentum=0.9, use_locking=True)
    else:
